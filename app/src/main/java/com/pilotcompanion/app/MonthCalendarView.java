...
final class MonthCalendarView extends View {
    ...
    MonthCalendarView(Context context, ScheduleRepository repository, ImportantDateRepository importantDates) {
        ...
        selected = repository.nearestScheduledDate(LocalDate.now());
    ...
    @Override protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        float cellWidth = getWidth() / 7f, header = dp(30), cellHeight = (getHeight() - header) / 6f;
        ...
        for (int i = 0; i < 7; i++) {
            ...
            canvas.drawText(day.getDisplayName(TextStyle.SHORT, Locale.US).toUpperCase(Locale.US), cellWidth * (i + .5f), dp(19), paint);
        }
        int offset = month.atDay(1).getDayOfWeek().getValue() % 7;
        for (int day = 1; day <= month.lengthOfMonth(); day++) {
            int index = offset + day - 1, row = index / 7, column = index % 7;
            float left = column * cellWidth, top = header + row * cellHeight;
            LocalDate date = month.atDay(day);
            if (date.equals(selected)) {
                paint.setColor(0xFF1F5A78);
                canvas.drawRoundRect(left + dp(2), top + dp(2), left + cellWidth - dp(2), top + cellHeight - dp(2), dp(9), dp(9), paint);
            }
            paint.setStyle(Paint.Style.STROKE); paint.setStrokeWidth(dp(1)); paint.setColor(0xFF27445F);
            canvas.drawRect(left, top, left + cellWidth, top + cellHeight, paint);
            paint.setStyle(Paint.Style.FILL); paint.setTextAlign(Paint.Align.LEFT); paint.setTextSize(dp(13));
            ...
            if (!legs.isEmpty()) {
                ...
                paint.setColor(stateColor); canvas.drawText(leg.origin() + ">" + leg.destination(), left + dp(5), y, paint); y += dp(12);
                paint.setColor(0xFFE8F1F8); canvas.drawText(leg.localTimes(), left + dp(5), y, paint); y += dp(12);
                paint.setColor(stateColor); canvas.drawText(leg.seatPosition() + " " + leg.changeLabel(), left + dp(5), y, paint); y += dp(12);
            }
            ...
            for (int i = 0; i < Math.min(events.size(), 2); i++) {
                ...
                paint.setTextSize(dp(7.5f)); paint.setColor(eventColor(event.type()));
                canvas.drawText(shortLabel(event.calendarLabel(), 18), left + dp(5), y, paint); y += dp(10);
            }
            if (events.size() > 2) {
                paint.setTextSize(dp(7.5f)); paint.setColor(0xFFB8C7D9);
    ...
    @Override public boolean onTouchEvent(MotionEvent event) {
        ...
        float header = dp(30); if (event.getY() < header) return true;
        ...
        if (day >= 1 && day <= month.lengthOfMonth()) {
            selected = month.atDay(day); listener.accept(selected); invalidate(); performClick();
        }
        return true;
    }
